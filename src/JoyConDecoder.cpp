...
constexpr uint64_t BUTTON_Z_MASK = 0x0000008000000000;   // Bit 21
constexpr uint64_t BUTTON_ZL_MASK = 0x000000800000;  // Bit 23
...
DS4_REPORT_EX GenerateNSOGCReport(const std::vector<uint8_t>& buffer)
{
    ...
    // Mappatura pulsanti
    if (state & BUTTON_A_MASK)        report.Report.wButtons |= DS4_BUTTON_CROSS;      // A -> Cross
    if (state & BUTTON_B_MASK)        report.Report.wButtons |= DS4_BUTTON_CIRCLE;     // B -> Circle
    if (state & BUTTON_X_MASK)        report.Report.wButtons |= DS4_BUTTON_SQUARE;     // X -> Square
    if (state & BUTTON_Y_MASK)        report.Report.wButtons |= DS4_BUTTON_TRIANGLE;   // Y -> Triangle
    
    // L e R sono analogici (sotto), Z e ZL sono digitali
    // Usa L_SHOULDER e R_SHOULDER per Z e ZL
    if (state & BUTTON_L_THUMB)       report.Report.wButtons |= DS4_BUTTON_THUMB_LEFT;
    if (state & BUTTON_R_THUMB)       report.Report.wButtons |= DS4_BUTTON_THUMB_RIGHT;
    if (state & BUTTON_BACK)          report.Report.wButtons |= DS4_BUTTON_SHARE;
    if (state & BUTTON_START)         report.Report.wButtons |= DS4_BUTTON_OPTIONS;
    if (state & BUTTON_GUIDE)         report.Report.bSpecial |= DS4_SPECIAL_BUTTON_PS;
    if (state & BUTTON_Z_MASK)   report.Report.wButtons |= DS4_BUTTON_SHOULDER_RIGHT;   // Z -> L1
    if (state & BUTTON_ZL_MASK)  report.Report.wButtons |= DS4_BUTTON_SHOULDER_LEFT;  // ZL -> R1
    ...
    // **ANALOG TRIGGERS L e R - VERSIONE FINALE**
    // Basato sui tuoi dati: byte 25 per L, byte 27 per R
    int ANALOG_L_POS = buffer.size() - 3;
    int ANALOG_R_POS = buffer.size() - 2;
    
    BYTE analogL = (buffer.size() > ANALOG_L_POS) ? buffer[ANALOG_L_POS] : 0;
    BYTE analogR = (buffer.size() > ANALOG_R_POS) ? buffer[ANALOG_R_POS] : 0;
    
    // Applica i trigger analogici L e R
    report.Report.bTriggerL = analogL;
    report.Report.bTriggerR = analogR;

    // DEBUG: Log dei valori quando cambiano
    static BYTE lastAnalogL = 0, lastAnalogR = 0;
    if (analogL != lastAnalogL || analogR != lastAnalogR) {
        if (analogL > 0 || analogR > 0) {
            printf("[ANALOG] L=%3d(@25) R=%3d(@27)\n", analogL, analogR);
        }
        lastAnalogL = analogL;
        lastAnalogR = analogR;
    }
